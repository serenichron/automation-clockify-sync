...
def collect_remote_sessions(machine: dict[str, Any], since: dt.datetime, until: dt.datetime, ssh_options: list[str]) -> dict[str, Any]:
    ...
    remote_code = "\nimport datetime as dt, json\nfrom pathlib import Path\nBUCHAREST=dt.timezone(dt.timedelta(hours=3))\nMACHINE=__MACHINE__\nCBASE=__CBASE__\nHBASE=__HBASE__\nSINCE=dt.datetime.fromisoformat(__SINCE__)\nUNTIL=dt.datetime.fromisoformat(__UNTIL__)\ndef parse_dt(s):\n    if not s: return None\n    try:\n        x=dt.datetime.fromisoformat(str(s).replace('Z','+00:00'))\n        return x if x.tzinfo else x.replace(tzinfo=BUCHAREST)\n    except Exception: return None\ndef local_str(x):\n    return x.astimezone(BUCHAREST).strftime('%Y-%m-%d %H:%M') if x else None\ndef label(path, base):\n    rel=str(path).replace(base,'').lstrip('/')\n    top=rel.split('/')[0]\n    for pref in ['-Users-blackthorne-Work-','-Users-blackthorne-','-home-blackthorne-Work-','-home-blackthorne-']:\n        if top.startswith(pref): return top[len(pref):]\n    return top\ndef skip_path(p):\n    return any(f in str(p) for f in ['/subagents/','paperclip-instances','claude-mem-observer','serenichron-paperclip-command'])\ndef parse_claude(p):\n    if skip_path(p): return []\n    first_user=''; last_assistant=''; ts=[]\n    try:\n        for line in Path(p).read_text(errors='ignore').splitlines():\n            try: o=json.loads(line)\n            except Exception: continue\n            t=parse_dt(o.get('timestamp'))\n            if not t or not (SINCE <= t.astimezone(BUCHAREST) < UNTIL): continue\n            msg_type=o.get('type')\n            c=o.get('message',{}).get('content','')\n            if isinstance(c,list):\n                text_parts=[]\n                for item in c:\n                    if isinstance(item,dict):\n                        if item.get('type')=='text': text_parts.append(item.get('text',''))\n                        elif item.get('type')=='tool_use': text_parts.append('[tool_use: '+item.get('name','?')+']')\n                        elif item.get('type')=='tool_result': text_parts.append('[tool_result]')\n                        elif item.get('type')=='thinking': text_parts.append('[thinking]')\n                c=' '.join(text_parts)\n            if msg_type=='user':\n                if not first_user: first_user=c[:200]\n                ts.append(t)\n            elif msg_type=='assistant':\n                last_assistant=c[:200]\n        if not ts: return []\n        ts.sort()\n        bursts=[]\n        start=end=ts[0]; count=1\n        for t in ts[1:]:\n            if (t-end).total_seconds()>1800:\n                bursts.append((start,end,count))\n                start=t; count=1\n            else: count+=1\n            end=t\n        bursts.append((start,end,count))\n        out=[]\n        for bs,be,cnt in bursts:\n            dur=max(1,int((be-bs).total_seconds()/60))\n            hb=all(t.minute in (29,44) for t in ts if bs<=t<=be)\n            out.append({'source':'claude','machine':MACHINE,'session_id':p.stem,'path':str(p),'label':label(p,CBASE),'start':local_str(bs),'end':local_str(be),'duration_minutes':dur,'user_messages':cnt,'heartbeat_like':hb,'first_user_message':first_user,'last_assistant_message':last_assistant,'evidence_level':'timestamps+user_message_count'})\n        return out\n    except Exception: return []\nout={'machine':MACHINE,'status':'ok','claude_bursts':[],'hermes_sessions':[],'errors':[]}\nif CBASE and Path(CBASE).exists():\n    for p in sorted(Path(CBASE).glob('*/*.jsonl')):\n        out['claude_bursts'].extend(parse_claude(p))\nelse:\n    out['errors'].append(f'claude_projects not found: {CBASE}')\nif HBASE and Path(HBASE).exists():\n    for p in sorted(Path(HBASE).glob('session_*.json')):\n        try:\n            obj=json.loads(p.read_text(errors='ignore'))\n            start=parse_dt(obj.get('session_start'))\n            last=parse_dt(obj.get('last_updated'))\n            touch=last or start\n            if not (touch and SINCE <= touch.astimezone(BUCHAREST) < UNTIL): continue\n            msgs=obj.get('messages',[])\n            uc=sum(1 for m in msgs if m.get('role')=='user')\n            tc=obj.get('message_count') or len(msgs)\n            if start and last: span=(last-start).total_seconds()/3600\n            else: span=0\n            if span>=4:\n                est=int(uc*7+(tc-uc)*0.5)\n                es=start or touch\n                ee=es+dt.timedelta(minutes=est)\n                ev=f'session spans {span:.1f}h but has only {uc} user msgs; estimated {est}m'\n            else:\n                es=start; ee=last; ev=f'session_start/last_updated used (span={span:.1f}h, {uc} user msgs)'\n            out['hermes_sessions'].append({'source':'hermes','machine':MACHINE,'session_id':obj.get('session_id') or p.stem,'path':str(p),'start':local_str(es),'end':local_str(ee),'real_span_hours':round(span,2),'user_messages':uc,'total_messages':tc,'model':obj.get('model'),'platform':obj.get('platform'),'evidence_level':ev})\n        except Exception as e:\n            out['errors'].append(f'hermes parse failed {p.name}: {e}')\nelse:\n    out['errors'].append(f'hermes_sessions not found: {HBASE}')\nprint(json.dumps(out))\n"
...
def route_meeting(meeting: dict[str, Any], routing: dict[str, Any]) -> dict[str, Any]:
    """Route a Fathom meeting to a Clockify project using meeting_routes."""
    title = (meeting.get("title") or "").strip()
    invitees = meeting.get("calendar_invitees", [])
    domains = {i.get("email", "").split("@")[-1] for i in invitees if i.get("email")}
    for r in routing.get("meeting_routes", []):
        domain_pat = r.get("email_domain", "")
        if domain_pat and domain_pat in domains:
            return {"action": "propose", **r}
        title_pat = r.get("title_regex", "")
        if title_pat and re.search(title_pat, title, re.IGNORECASE):
            return {"action": "propose", **r}
    ...
    if not external:
        return {
            ...
            "tag_suffixes": ["35aa9aef"],
            "tag_names": ["Project Management"],
            "billable": True,
        }
    return {"action": "ambiguous", "reason": "No meeting route matched"}
...
def build_proposals(evidence: dict[str, Any], routing: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    # --- Fathom meeting proposals ---
    for m in evidence.get("fathom", {}).get("meetings", []):
        start = m.get("start")
        end = m.get("end")
        if not start or not end:
            continue
        title = (m.get("title") or "").strip()
        if overlaps_existing(m, existing):
            skipped.append({"id": f"fathom-{m.get('recording_id','?')}", "source": "fathom", "time": f"{start}–{end}", "label": title, "reason": "covered by existing Clockify entry overlap"})
            continue
        route = route_meeting(m, routing)
        if route["action"] == "skip":
            skipped.append({"id": f"fathom-{m.get('recording_id','?')}", "source": "fathom", "time": f"{start}–{end}", "label": title, "reason": route.get("reason", "route skip")})
            continue
        if route["action"] == "ambiguous":
            ambiguous.append({"id": f"A{len(ambiguous)+1:03d}", "source": "fathom", "time": f"{start}–{end}", "label": title, "reason": route.get("reason"), "machine": "fathom"})
            continue
        cs = parse_dt(start)
        ce = parse_dt(end)
        duration_minutes = max(1, int((ce - cs).total_seconds() / 60)) if cs and ce else None
        cand = {
            ...
            "source_label": title,
            "confidence": "high",
            "description": f"{route.get('project_name')} — {title}",
            "rationale": f"Fathom meeting: {title}",
        }
        proposals.append(cand)

    # --- Session-based proposals ---
