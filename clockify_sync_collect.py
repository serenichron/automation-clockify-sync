...
def _make_description(project: str, burst: dict[str, Any], confidence: str = "medium") -> str:
    """Build a human-readable description from session context.

    Format: [PREFIX] — [concise action summary]
    Prefixes: SC for Serenichron, TSTP for TST Prep (no level in description).
    ...
    # Determine prefix
    if "tst prep" in project.lower() or "tstprep" in project.lower():
        prefix = "TSTP"
    else:
        prefix = "SC"
    ...
    for machine in evidence.get("sessions", []):
        for b in machine.get("claude_bursts", []):
            ...
            cand = {
                ...
                "description": _make_description(route.get('project_name'), b, route.get('confidence', 'medium')),
        ...
        for cs in machine.get("codex_sessions", []):
            ...
            cand = {
                ...
                "description": f"{route.get('project_name')} — {cs.get('title') or 'Codex session'}",
